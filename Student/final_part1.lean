...
There exists some natural number n such that the square of n is 4.
-/
/-
Since we only have to show that one such n exists, we can prove this proposition by
presenting a witness whose square is 4, 2 in this case, along with a proof that said
witness does indeed yield 4 when squared. The latter is obtained by reducing the
proposition 4 = 2 ^ 2 to 4 = 4 and applying the reflexive property of eqaulity to
recognize that 2 and 4 satisfy that proposition and thus the square relation.
-/
